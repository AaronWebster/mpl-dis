...
Consider a random phasor sum consisting of $N$ phasors, depicted schematically
in \Figure{fig:phasorarg1}.  In the case of single scattering, the $N$ phasors
map directly to $N$ scatterers, and the problem is equivalent to a random walk
in $\mathbb{R}^2$.  Assuming all scattering paths equally probable, all
phasors have equal magntude.  For the sake of this argument, the phasor
magnitude is assumed to be unity, whereby the mean value of the phasor sum is
$\sqrt{N}$.  Upon the addition of an additional ($N+1$) scatterer, the
magnitude of the phasor sum for single scattering is therefore altered by a
mean value of $1/N$.

Consider next the consequence of the addition of higher scattering orders.
First, the`:

andom phasor sum where each phasor is due
to a double scattering process --- each phasor is the result of a path which
visits two scatterers.  For $N$ scatterers, there are $N^2$ possible phasors 

If each component in the phasor sum
is due to single scattering

Upon the addition of a single
scatterer, the magnitude of the phasor sum 
