...
single scatterer depends on strongly the underlying scattering process;
whether it is in the single or multiple scattering regime.  In the purely
single scattering regime the result is straightforward.  Since each
component in the random phasor sum (\Equation{eqn:phasorsum}) contributes
equally, the addition of a single phasor will cause a change in the 
...
(the pearsons introduces whatever)
(we're not smart enough to do multiple scattering stuff, so here you go)
