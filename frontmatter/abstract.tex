...
are two most ubiquitous biotransducers: the optical surface plasmon resonance
(SPR), and the mechanical piezoelectric quartz crystal microbalance (QCM).
These two devices, the subject of the present work, both possess novel modes
of operation which extend their versatility to other bio-relevant sensing
applications.

In the optical regime, surface plasmon polaritons (SPPs) excited evanescently
by light are by far the most popular label-free affinity biotransducer for
monitoring bulk refractive index changes.  The sensitivity of SPR is primarily
due to the field enhancement by SPPs on the sensor surface, however SPPs
themselves also possess high spatial resolution beyond the diffraction limit;
a property typically absent as specific feature of the SPR sensorgram.  In
this work, it is shown that by considering optical speckle from single and
multiply scattered SPPs inherent in the SPR signal itself, an entirely new set
...
of single nanoparticles can be resolved in an unmodified SPR setup, whereby
the breadth of SPR experiments may be extended to encompass both bulk sensing
...
changes in the QCM sensorgram, an issue complicated by choice of theoretical
model.  Here it is shown that application of controlled centrifugal forces in
a QCM assay has a profound utility in revealing the underlying biomechanical
properties of a sample.  This \textit{centrifugal force quartz crystal
microbalance} concept works by modifying the QCM-sample coupling mechanism.
Centrifugal force is demonstrated to be useful in not only to enhanceing the
sensitivity of a traditional QCM measurement, but also in obtaining the
sample's complex biomechanical properties repeatedly \textit{in situ} and in
real time.  \end{abstract}
