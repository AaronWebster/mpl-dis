...
FT1ab = fit(theta',-1.*TP1ab','smoothingspline');
t1ab = fminbnd(FT1ab,ta,tb);
FT2ab = fit(theta',-1.*TP2ab','smoothingspline');
...
% display the sensitivity calculations
fprintf('cone angular:    %d\n',abs(t1ab-t2ab)/dn)
fprintf('notch angular:   %d\n',abs(r1a-r2a)/dn)
fprintf('cone intensity:  %d\n',max(abs(TP1ab-TP2ab))/dn)
fprintf('notch intensity: %d\n',max(abs(RP1a-RP2a))/dn)
...
subplot(121)
hold on;
plot(theta,FR1a(theta),'Color',mycolor(1,:))
plot(theta,FR2a(theta),'Color',mycolor(2,:))
xlabel('angle $\theta$ [deg]')
ylabel('$|r^p|^2$')
title('specular')
legend('$\Delta n=0$',sprintf('$\\Delta n=%.3g$',dn))
subplot(122)
hold on;
plot(theta,-FT1ab(theta),'Color',mycolor(6,:))
plot(theta,-FT2ab(theta),'Color',mycolor(5,:))
xlabel('angle $\theta$ [deg]')
...
%% save to tikz
if false
filename = sprintf('sens_angular_asp.tex');
