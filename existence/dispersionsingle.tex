...
condition under which the momentum the incident light is matched to that of
an SPP at a particular frequency.  For a photon in a dielectric $\omega = c
k /\sqrt{\epsilon_i}$ for $i=1,2$.  This is known as the ``light line''.
The SPP on the other hand lies below this and asymptotically approaches
...
Atenuated Total Reflection (ATR) configuration.  To be sure, there are a
multitude of different excitation strategies beyond this one, however for
our experiments we will focus only on ATR.  This choice is guided primarily
by ease of fabrication with the facilities we have on hand (a sputtering
machine and a metal target, see \Chapter{ch:experimental}).
