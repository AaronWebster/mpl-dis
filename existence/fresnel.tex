...
\caption{Prism coupled excitation of SPPs in the Kretschmann configuration,
								(a), a three layer interface system, and (b), a four layer
								interface system. }
\label{fig:prismcoupledsetups}
\end{figure}

To begin, consider the single interface system, first shown in
...
at $\lambda=\SI{660}{\nano\meter}$, from which it is straighforward to
...
The Fresnel equations have thus far been used to predict the conditions for
...
taking into account effects from SPP propagation.  
...
\Section{sec:mrfsim}.  Note that even though the Fresnel equations provides 
...
four layer structure, designated ``long range SPPs'' (LRSPPs).  The choice
of this terminology is motivated by the respective SPP propagation lengths.
Figures~\ref{fig:fresnelnearfieldspp} and~\ref{fig:fresnelnearfieldlrspp} were
both computed using an excitation wavelength of \SI{660}{\nano\meter} and a
Gaussian incident beam of numerical aperture $\mathrm{NA}=0.2$. 

As evident from the coupling conditions predicted by the Fresnel equations,
the most prominent optical feature of SPP excitation is a minimum, or
``notch'' in the specularly reflected light at the surface plasmon
resonance angle $\thetasp$.  The angular location of the minimum is highly
sensitive to the refractive index in which the SPP propagates; the
...
along the metal interface at $z=0$ in the positive $x$ direction.  Here it
can be observed that the LRSPP propagation length is nearly 10 times that
of conventional SPPs.  Such increased propagation lengths aren't
necessarily connected to the system's inherent bulk refractive index
sensitivity, though in practice physical instrumentation provides a better
signal with sharper optical resonances compared with broad ones.  

%Questions of bulk refractive index sensitivity are further expounded in \Chapter
