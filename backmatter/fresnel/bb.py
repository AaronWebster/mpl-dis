...
def sellmeier(material, lambda0):
	if material == 'BK7':
		return sqrt( 1 + 1.03961212*pow(lambda0*1e6,2)/(pow(lambda0*1e6,2)-0.00600069867) + 0.231792344*pow(lambda0*1e6,2)/(pow(lambda0*1e6,2)-0.0200179144) + 1.01046945*pow(lambda0*1e6,2)/(pow(lambda0*1e6,2)-103.560653))
	elif material == 'LAH79':
		return sqrt( 1 + 2.32557148E+00*pow(lambda0*1e6,2)/(pow(lambda0*1e6,2)-1.32895208E-02) + 5.07967133E-01*pow(lambda0*1e6,2)/(pow(lambda0*1e6,2)-5.28335449E-02) + 2.43087198E+00*pow(lambda0*1e6,2)/(pow(lambda0*1e6,2)-1.61122408E+02) )

def LDBB(metal, model, lambda0):

	metals = ['Ag', 'Au', 'Cu', 'Al', 'Be', 'Cr', 'Ni', 'Pd', 'Pt', 'Ti', 'W']

	# material data for the LD model
	LDdata = array([
	...
	# experimental data for the BB model
	BBdata = array([
	[9.01,9.03,10.83,14.98,18.51,10.75,15.92,9.72,9.59,7.29,13.22],
...
#f = open('%s_er_BB.dat' % metals[idx] ,'w')
#for i in range(0,N):
#	f.write("%.18e\t%.18e\n" % (lambda0[i], real(epsilon[i])) )
#f.close()

#f = open('%s_ei_BB.dat' % metals[idx] ,'w')
#for i in range(0,N):
#	f.write("%.18e\t%.18e\n" % (lambda0[i], imag(epsilon[i])) )
#f.close()

# complex index of refraction
#q = (1/sqrt(2))*sqrt(sqrt(real(epsilon)**2+imag(epsilon)**2)+real(epsilon)) + 1.0J*sqrt(sqrt(real(epsilon)**2+imag(epsilon)**2)-real(epsilon))
#f = open('%s_nr_BB.dat' % metals[idx] ,'w')
#for i in range(0,N):
#	f.write("%.18e\t%.18e\n" % (lambda0[i], real(q[i])) )
#f.close()

#f = open('%s_ni_BB.dat' % metals[idx] ,'w')
#for i in range(0,N):
#	f.write("%.18e\t%.18e\n" % (lambda0[i], imag(q[i])) )
#f.close()
#
