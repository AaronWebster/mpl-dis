#!/usr/bin/env python3
"""
Generate TikZ figure for QCM measurements vs time (4 subplots).
...
MATLAB code analysis (showtempair.m):
  ...
  Line 36-39: matlab2tikz('showtempair.tex', height='2.5cm', width='12cm')
...
def generate_showtempair():
    """
    Generate showtempair.tex - rigorously replicates MATLAB showtempair.m
    ...
    # Load data (matching MATLAB: tmp = load('data/QCM00431b-freq.txt'))
    freq_data = np.loadtxt(data_dir / 'QCM00431b-freq.txt')
    res_data = np.loadtxt(data_dir / 'QCM00431b-res.txt')
    temp_data = np.loadtxt(data_dir / 'QCM00431b-temp.txt')
    ang_data = np.loadtxt(data_dir / 'QCM00431b-ang.txt')
    ...
    with open(output_file, 'w') as f:
        ...
        write_tikz_subplot(f, time_freq_ds, val_freq_ds, color_blue, '$\\Delta\\,f$',
                          'time [s]', '$\\Delta\\,f$ [Hz]', 
                          ymin=-1.0, ymax=4.0, width='12cm', height='2.5cm',
        ...
        write_tikz_subplot(f, time_res_ds, val_res_ds, color_red, '$\\Delta\\Gamma$',
                          ...
                          ymin=-0.05, ymax=0.2, width='12cm', height='2.5cm',
