...
\SI{30}{\percent} \ce{H2O2}) for \SI{5}{\minute} and rinsed liberally with
pure water.
...
value~\cite{valdois2794influence} of $\df/\Delta g
	=\SI{2.188(6)e-2}{\hertz\per\g}$ (see
\Table{tbl:environmentaleffects}).  The CF-QCM data shows similar
behavior: $\df/\Delta g =\SI{2.682(23)e-2}{\hertz\per\g}$ in the loading
...
acceleration are known, the data predicts a linear relationship of $\dg/\Delta g =
	\SI{9.203(171)e-4}{\hertz\per\g}$.  The slight increase in $\df/\Delta g$
...
load is linear and smaller than that of air: $\df/\Delta g=
	\SI{1.357(24)e-2}{\hertz \per\g}$ and $\dg/\Delta g=
	\SI{2.865(73)e-3}{\hertz\per\g}$ as found by linear least squares fitting.

It is obvious that acceleration-dependent forces in the liquid phase are not
...
As first referenced in \Figure{fig:expsetup}\,(b), the frequency and bandwidth
shifts of free particles in the liquid phase as a function of g-force is shown
in \Figure{fig:loadplot}\,(c).  Free streptavidin-coated polystyrene particles
...
To attach oligos to the \gls{qcm}. they were first immersed in a
...
decrease ($\dg=\SI{-2.9120(95)}{\hertz\per\g}$) in the bandwidth as
...
was observed for STE buffer alone ($\df=\SI{-0.3260(29)}{\hertz\per\g}$,
...
\Figure{fig:expsetup} and \Figure{fig:loadplot}.  The predictions for
particles with known diameter $\bar{d}_\mathrm{actual}=1, 2,
	15,\,\mathrm{and}\;\SI{25}{\micro\meter}$ are shown in
\Table{tbl:particlesize}.
